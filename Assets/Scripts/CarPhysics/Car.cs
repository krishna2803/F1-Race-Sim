...
public class Car : MonoBehaviour
{
    ...
    public float SpeedKilometersPerHour
    {
        get
        {
            return carRigidBody2D.velocity.magnitude * 18f / 5f;
        }
    }
    ...
    float accelerationInput; // Throttle
    ...
    void Init()
    {
        ...
        Inertia = carRigidBody2D.mass * InertiaScale;

        // Set starting angle of car
        carRigidBody2D.rotation = transform.rotation.eulerAngles.z;
        HeadingAngle = (carRigidBody2D.rotation + 90) * Mathf.Deg2Rad;
    }

    void Start()
    {

        AxleFront.Init(carRigidBody2D, WheelBase);
        AxleRear.Init(carRigidBody2D, WheelBase);
    ...
    void Update()
    {
        ...
        if (IsPlayerControlled)
        {
            ...
            SteerDirection = SmoothSteering(steeringInput * 2.0f);
        ...
        Engine.UpdateAutomaticTransmission(carRigidBody2D);
    ...
    void FixedUpdate()
    {
        ...
        Velocity = carRigidBody2D.velocity;
        HeadingAngle = (carRigidBody2D.rotation + 90) * Mathf.Deg2Rad;
        ...
        // Weight on each axle
        float weightFront = carRigidBody2D.mass * (AxleFront.WeightRatio * -Physics2D.gravity.y - transferX);
        float weightRear = carRigidBody2D.mass * (AxleRear.WeightRatio * -Physics2D.gravity.y + transferX);
        ...
        float activeThrottle = (accelerationInput * Engine.GetTorque(carRigidBody2D)) * (Engine.GearRatio * Engine.EffectiveGearRatio);
        ...
        // Acceleration
        LocalAcceleration.x = totalForceX / carRigidBody2D.mass;
        LocalAcceleration.y = totalForceY / carRigidBody2D.mass;
        ...
        carRigidBody2D.velocity = Velocity;

        carRigidBody2D.MoveRotation(Mathf.Rad2Deg * HeadingAngle - 90);

        // velocity vs up update
    ...
    void ohno_OnGUI()
    {
        if (IsPlayerControlled)
        {
            ...
            GUI.Label(new Rect(5, 25, 300, 20), "RPM: " + Engine.GetRPM(carRigidBody2D).ToString());
            ...
            GUI.Label(new Rect(5, 165, 300, 20), "Throttle: " + accelerationInput.ToString());
    ...
    public bool IsTireScreeching(out float lateralVelocity, out bool isBraking)
    {
        ...
        // this value can be adjusted according to your preferences
