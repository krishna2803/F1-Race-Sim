            ...
            if (sc.Spline.ControlPointsModifiable)
            {
                if (sc.Spline.SplineCurveType == CurveType.Bezier)
                {
                    for (int i = 0; i < sc.Spline.ControlPoints.Count; i += 3)
                    {
                        ...
                        var fmh_383_65_638477759203173601 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i], 8f, Vector3.zero,
                        ...
                        var fmh_394_65_638477759203200407 = Quaternion.identity; newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i + 2], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                        ...
                        //Draw the site point
                        Handles.color = Color.red;
                        var fmh_408_65_638477759203203022 = Quaternion.identity; newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i + 1], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                ...
                else if(sc.Spline.SplineCurveType == CurveType.CatmullRom)
                {
                    for (int i = 0; i < sc.Spline.ControlPoints.Count; i++)
                    {
                        //Draw the start and end control point
                        Handles.color = Color.red;
                        var fmh_436_65_638477759203206811 = Quaternion.identity; Vector3 newPos = Handles.FreeMoveHandle(unApplliedControlPoints[i], 8f, Vector3.zero,
                                                                Handles.CylinderHandleCap);
                        //if newPos is not the same as the control point on the spline 
                        if (sc.Spline.ControlPoints[i] != newPos)
                        {
